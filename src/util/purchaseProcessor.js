...
class PurchaseProcessor {
  ...
  start = async () => {
    ...
    this.#getPurchasesInterval = setInterval(
      this.#getAllPurchases,
      this.#getPurchaseInterval,
    );
    this.#doNext();
  };
  ...
  #getNewPurchasesForUser = async (steamId) => {
    ...
    return got(`http://${process.env.API_URL}/api/purchases/${steamId}`)
      .then((res) => {
        ...
        console.log(
          `Pulling games list from the steam api for user ${steamId}`,
        );
      ...
      .then((res) => {
        ...
        return Promise.resolve(
          this.#filterNewPurchases(gamesList.new, gamesList.current),
        );
      })
      .catch((err) => {
        console.error(err);
  ...
  #processPurchase = () => {
    ...
    SteamApi.getAppDetails(nextItem.appid).then((res) => {
      const steamResult = res[nextItem.appid];
      if (!steamResult.success || !steamResult.data) {
        console.warn(
          `Received and invalid steam api response for ${nextItem.appid}`,
        );
      ...
      return got
        ...
        .then((res) => {
          ...
          if (statusCode >= 200 && statusCode < 300) {
            ...
            console.error(
              `Server error while adding \"${purchase.name}\" (${purchase.appId})`,
            );

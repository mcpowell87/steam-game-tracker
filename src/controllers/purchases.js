...
export const get = async (req, res) => {
  ...
  try {
    if (req.query.n) {
      ...
      purchases = await getPurchasesByDateRange(
        req.params.steamId,
        req.query.start,
        req.query.end,
      );
...
export const stats = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "There was a problem while processing statistics." });
  }
};
...
export const create = async (req, res) => {
  ...
  if (exists) {
    res
      .status(409)
      .json({
        message: `AppId '${purchases.appId}' already found in the database`,
      });
    return;
  }

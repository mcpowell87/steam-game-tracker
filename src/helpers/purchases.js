...
export const getPurchasesByDateRange = (steamId, start, end) => {
  ...
  if (start) {
    ...
    if (end) {
      ...
      endDate = DateTime.fromISO(start)
        .setZone("America/New_York")
        .endOf("day")
        .toISO();
    }
...
export const getPurchaseByAppId = (steamId, appId) => {
  ...
  return Purchases.findOne(filter)
    .exec()
    .then((err, purchase) => {
      if (err) {
        console.error(err);
...
export const calculateStats = async (steamId) => {
  ...
  stats.costPerMinute = +(stats.totalCost / stats.totalPlaytimeMinutes).toFixed(
    2,
  );
  stats.percentagePlayed = (
    (stats.numberPlayed / stats.numberOwned) *
    100
  ).toFixed(2);
